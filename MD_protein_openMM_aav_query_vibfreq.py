...
# read MD ctl file
infile = open("MDq_aav_vibfreq.ctl", "r")
...
writePath= "./MDsimulationLOG_vibfreq.txt"
...
if(antechamber == "no"):      
    print ('MD equilibration run for', 'eq_'+PDBid+'_vibfreq.nc')
    simulation.reporters.append(pmd.openmm.NetCDFReporter('eq_'+PDBid+'_vibfreq.nc', INTsamp))
    simulation.reporters.append(app.StateDataReporter(stdout, 1000, step=True, potentialEnergy=True, temperature=True, progress=True, remainingTime=False, speed=False, totalSteps=TIMEeq, separator='\t'))
    print('Equilibrating...')
    simulation.step(TIMEeq) # no separate heating step and fixed equilibration time of 0.1ns
    #simulation.step(1000) # for testing
    print('eq_'+PDBid+'_vibfreq.nc is done')
    fin_pos = simulation.context.getState(getPositions=True).getPositions() 
    # end reporter
    pmd.openmm.NetCDFReporter.finalize('eq_'+PDBid+'_vibfreq.nc')
    print ('MD production run for', 'prod_'+PDBid+'_vibfreq.nc')
    simulation.reporters.append(pmd.openmm.NetCDFReporter('prod_'+PDBid+'_vibfreq.nc', INTsamp))
    ...
    print('prod_'+PDBid+'_vibfreq.nc is done')
    # end reporter
    pmd.openmm.NetCDFReporter.finalize('prod_'+PDBid+'_vibfreq.nc')
if(antechamber == "yes"): 
    print ('MD equilibration run for', 'eq_'+PDBid+'_complex_vibfreq.nc')
    simulation.reporters.append(pmd.openmm.NetCDFReporter('eq_'+PDBid+'_complex_vibfreq.nc', INTsamp))
    simulation.reporters.append(app.StateDataReporter(stdout, 1000, step=True, potentialEnergy=True, temperature=True, progress=True, remainingTime=False, speed=False, totalSteps=TIMEeq, separator='\t'))
    print('Equilibrating...')
    simulation.step(TIMEeq) # no separate heating step and fixed equilibration time of 0.1ns
    #simulation.step(1000) # for testing
    print('eq_'+PDBid+'_complex_vibfreq.nc is done')
    fin_pos = simulation.context.getState(getPositions=True).getPositions()
    # end reporter
    pmd.openmm.NetCDFReporter.finalize('eq_'+PDBid+'_complex_vibfreq.nc')
    print ('MD production run for', 'prod_'+PDBid+'_complex_vibfreq.nc')
    simulation.reporters.append(pmd.openmm.NetCDFReporter('prod_'+PDBid+'_complex_vibfreq.nc', INTsamp))
    #simulation.reporters.append(app.StateDataReporter(stdout, 10000, step=True, potentialEnergy=True, temperature=True, progress=True, remainingTime=False, speed=False, totalSteps=TIMEprod, separator='\t'))
    # run production simulation
    print('Running Production...')
    simulation.context.setPositions(fin_pos)
    simulation.step(TIMEprod)
    print('prod_'+PDBid+'_complex_vibfreq.nc is done')
    # end reporter
    pmd.openmm.NetCDFReporter.finalize('prod_'+PDBid+'_complex_vibfreq.nc')
